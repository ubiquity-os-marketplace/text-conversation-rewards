...
type BaseCsvRecordData = Omit<
	CsvRecord,
	"ItemType" | "ItemID" | "ItemURL" | "Reward" | "CommentType"
>;
...
async function readResultsDirectory(dirPath: string): Promise<string[]> {
	try {
		...
		return files.filter(
			(file) => file.endsWith(".json") && file !== invalidIssuesFilename,
		);
	} catch (error) {
...
function parseFilename(
	filename: string,
): { organization: string; repository: string; issueNumber: string } | null {
...
async function readAndParseJsonFile(filePath: string): Promise<Result | null> {
	try {
		...
		console.error(
			`Error reading or parsing JSON file ${path.basename(filePath)}:`,
			error,
		);
		return null;
	}
...
function extractTaskRecord(
	userData: UserResultData,
	baseRecord: BaseCsvRecordData,
): CsvRecord | null {
	if (!userData.task) {
		return null;
	}
...
function extractCommentRecords(
	userData: UserResultData,
	baseRecord: BaseCsvRecordData,
): CsvRecord[] {
