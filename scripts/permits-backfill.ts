...
async function backfillPermits() {
	...
	for (const permit of permitsData) {
		...
		if (userError || !userData?.wallet_id) {
			console.log(
				`Permit ${permit.id}: user ${permit.beneficiary_id} not found`,
			);
			continue;
		}
		...
		if (userWalletError || !userWalletData?.address) {
			console.log(
				`Permit ${permit.id}: wallet for user ${permit.beneficiary_id} not found`,
			);
			continue;
		}
		...
		).exec(locationData.node_url);
		if (!match) {
			console.log(
				`Permit ${permit.id}: invalid location node_url: ${locationData.node_url}`,
			);
			continue;
		}
		...
		for (const comment of comments) {
			...
			matchedPermit = findMatchingPermitRewardInComment(
				comment.body,
				permit.signature,
				userAddress,
			);
		...
		if (!matchedPermit) {
			console.log(
				`Permit ${permit.id}: no matching reward found (signature: ${permit.signature})`,
			);
			continue;
		}
		...
		const tokenId = await getOrCreateToken(
			matchedPermit.tokenAddress,
			matchedPermit.networkId,
		);
		if (!tokenId) {
...
async function fetchGithubComments(
	...
	try {
		...
		if (error instanceof RequestError) {
			if (error.status === 404) {
				console.log(
					`GitHub ${owner + "/" + repo + "/issues/" + issueNumber} not found, skipping.`,
				);
				return [];
			} else if (
				error.status === 403 &&
				...
				const waitTime =
					parseInt(error.response.headers["x-ratelimit-reset"] ?? "2") * 1000 -
					Date.now();
				console.log(
					`GitHub API rate limit hit, waiting ${waitTime / 1000} seconds...`,
				);
				await new Promise((res) => setTimeout(res, waitTime));
				return fetchGithubComments(owner, repo, issueNumber);
			}
			console.error(
				`Error fetching GitHub comments for issue ${issueNumber}:`,
				error,
			);
			return [];
		}
...
async function getOrCreateWallet(address: string): Promise<number | null> {
	...
	if (insertError || !inserted) {
		console.error(
			`Error inserting new wallet with address ${address}`,
			insertError,
		);
		return null;
	}
...
async function getOrCreatePartner(walletId: number): Promise<number | null> {
	...
	if (insertError || !inserted) {
		console.error(
			`Error inserting new partner for wallet_id ${walletId}`,
			insertError,
		);
		return null;
	}
...
async function getOrCreateToken(
	...
	if (findError) {
		console.error(
			`Error finding token with address ${address} and network ${network}`,
			findError,
		);
		return null;
	}
	...
	if (insertError || !inserted) {
		console.error(
			`Error inserting new token with address ${address} and network ${network}`,
			insertError,
		);
		return null;
	}
...
export async function getMinimizedCommentStatus(
	...
	for (let i = 0; i < comments.length; i += CHUNK_SIZE) {
		...
		const commentsData = await _octokit.graphql<{ nodes?: IssueComment[] }>(
			QUERY_COMMENT_DETAILS,
			{
