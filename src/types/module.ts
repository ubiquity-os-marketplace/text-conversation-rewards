...
export function isPullRequest(
	context: Pick<ContextPlugin, "payload">,
): boolean {
	return (
		"pull_request" in context.payload ||
		("issue" in context.payload && !!context.payload.issue.pull_request)
	);
}
...
export abstract class BaseModule implements Module {
	...
	protected async computePriority(
		data: Readonly<IssueActivity>,
	): Promise<number> {
		try {
			...
			throw this.context.logger.warn(
				"Failed to compute the averaged priority.",
				{ e },
			);
		}
	}
