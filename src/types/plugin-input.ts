...
const rewardSettingsType = T.Object(
	{
		...
		evmPrivateEncrypted: T.String({
			description:
				"The encrypted Ethereum private key to use for funding rewards.",
			examples: ["0x000..."],
		}),
		erc20RewardToken: T.String({
			default: "0xC6ed4f520f6A4e4DC27273509239b7F8A68d2068",
		}),
	},
		...
		incentives: T.Object(
			{
				closeTaskReward: T.Object(
					{
						rewardAmount: T.Number({
							default: 5,
							description:
								"Reward amount that the user should get on closing a stale task",
						}),
						stalenessDuration: T.String({
							default: "30 days",
							description:
								"Minimum time an issue must stay open before the close-task reward applies",
						}),
					},
					{ default: {} },
				),
				shouldProcessUnlinkedPullRequests: T.Boolean({
					default: false,
					description:
						"Should pull-requests that are not linked to an issue be processed?",
				}),
				/**
				...
				limitRewards: T.Boolean({
					default: true,
					description:
						"Should the rewards of non-assignees be limited to the task reward?",
				}),
				collaboratorOnlyPaymentInvocation: T.Boolean({
					default: true,
					description: "If false, will allow contributors to generate permits.",
				}),
				contentEvaluator: T.Union(
					[contentEvaluatorConfigurationType, T.Null()],
					{ default: null },
				),
				...
				reviewIncentivizer: T.Union(
					[reviewIncentivizerConfigurationType, T.Null()],
					{ default: null },
				),
				eventIncentives: T.Union([eventIncentivesConfigurationType, T.Null()], {
					default: null,
				}),
				simplificationIncentivizer: T.Union(
					[simplificationIncentivizerConfigurationType, T.Null()],
					{ default: null },
				),
				formattingEvaluator: T.Union(
					[formattingEvaluatorConfigurationType, T.Null()],
					{ default: null },
				),
				payment: T.Union([paymentConfigurationType, T.Null()], {
					default: null,
				}),
				...
				externalContent: T.Union([externalContentConfigurationType, T.Null()], {
					default: null,
				}),
			},
			{ default: {} },
...
export type SupportedEvents =
	...
	| "pull_request.closed";
export type ContextPlugin<T extends SupportedEvents = SupportedEvents> =
	Context<PluginSettings, EnvConfig, Command, T> & {
