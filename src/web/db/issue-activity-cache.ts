			...
			try {
				const issue =
					"issue" in this._context.payload
						? this._context.payload.issue
						: this._context.payload.pull_request;
				...
				this._context.logger.debug(`Fetched related issues from the cache.`, {
					relatedIssue,
				});
				if (!relatedIssue) {
					await super.init();
				...
				if (error && typeof error === "object" && "errno" in error) {
					...
					throw this._context.logger.error(
						`Error fetching related issues from the database`,
						{
