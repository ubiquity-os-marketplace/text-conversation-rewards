...
async function getRepositoryList(context: ContextPlugin, orgName: string) {
	...
	return repositories.filter(
		...
		(repo) =>
			!repo.owner.login.includes("ubiquity") ||
			!repo.name.includes("devpool-directory"),
	);
}
...
async function parseIssues(
	...
	for (const issue of issues) {
		...
		if (existsSync(filePath) && !isSingleIssueMode) {
			...
			const activity = new IssueActivityCache(
				ctx,
				issueElem,
				"useCache" in config,
			);
...
>(
	async (context) => {
		...
		for (const repo of repositories) {
			...
			results.push(
				...(await parseIssues(pluginContext, issues, repo, isSingleIssueMode)),
			);
		}
...
function detectLlmModule(
	text: { messages?: Array<{ content?: unknown }> },
	moduleParam?: string,
) {
			...
			case "llmWebsiteModel":
				return Response.json({
					choices: [
						{
							message: {
								content: "This website contains some relevant content.",
							},
						},
					],
				});
			case "llmImageModel":
				return Response.json({
					choices: [
						{
							message: {
								content: "This image represents some relevant content.",
							},
						},
					],
				});
