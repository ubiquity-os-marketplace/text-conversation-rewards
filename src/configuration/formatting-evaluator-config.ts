...
export const commentType = stringLiteralUnion(
	Object.keys(CommentKind).flatMap((kind) =>
		Object.keys(CommentAssociation).map(
			(association) => `${kind}_${association}`,
		),
	) as CommentType[],
);
...
const htmlType = Type.Record(Type.String(), htmlEntity, {
	...
	default: {
		...
		b: { score: 0, countWords: true },
		summary: { score: 0, countWords: false },
		details: { score: 0, countWords: false },
		table: { score: 0, countWords: false },
	},
});
...
export const formattingEvaluatorConfigurationType = Type.Object(
	{
		...
		readabilityScoring: Type.Object(
			{
				...
				weight: Type.Number({
					default: 0.3,
					description:
						"Weight of readability score in the total formatting score",
				}),
				targetReadabilityScore: Type.Number({
					default: 60,
					description:
						"Ideal Flesch-Kincaid score (60-70 is considered ideal for general audience)",
				}),
			},
			{ default: {} },
		),
		...
		multipliers: Type.Transform(
			Type.Array(
				...
				{
					description:
						"Multipliers applied to different parts of the comment according the role of the author",
					default: [
						{
...
export type FormattingEvaluatorConfiguration = Static<
	typeof formattingEvaluatorConfigurationType
>;
