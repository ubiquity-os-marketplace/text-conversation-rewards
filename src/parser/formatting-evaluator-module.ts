...
type IssueEditNode =
	IssueEdits["repository"]["issue"]["userContentEdits"]["nodes"][number];
...
export class FormattingEvaluatorModule extends BaseModule {
	...
	constructor(context: ContextPlugin) {
		...
		if (this._configuration?.multipliers) {
			this._multipliers = this._configuration.multipliers.reduce(
				(acc, curr) => {
					return {
						...acc,
						...curr.role.reduce(
	...
	private _calculateFleschKincaid(text: string): ReadabilityScore {
		...
		if (rawFleschKincaid > 100) {
			...
			const distance = Math.abs(
				rawFleschKincaid -
					(this._readabilityConfig?.targetReadabilityScore ?? 60),
			);
	...
	async _getOriginalAuthorshipPercentage(
		...
		const data = await this.context.octokit.graphql.paginate<IssueEdits>(
			QUERY_ISSUE_EDITS,
			{
				owner,
				repo,
		...
		const userEdits = data.repository.issue.userContentEdits.nodes.sort(
			(a: IssueEditNode, b: IssueEditNode) => {
		...
		if (!userEdits.length) {
			this.context.logger.debug(
				"No edits detected on the issue body, skipping.",
			);
			return 1;
		}
	...
	async transform(data: Readonly<IssueActivity>, result: Result) {
		for (const key of Object.keys(result)) {
			...
			for (const comment of comments) {
				const { formatting, words, readability } =
					await this._getFormattingScore(comment);
				const multiplierFactor = this._multipliers?.[comment.commentType] ?? {
					multiplier: 0,
				};
				...
				const authorship = await this._getOriginalAuthorshipPercentage(
					key,
					comment,
				);
				const reward = (
					comment.score?.reward
						? formattingTotal.add(comment.score.reward)
						: formattingTotal
				)
					.mul(authorship)
					.toNumber();
	...
	private _calculateFormattingResult(
		formatting: Awaited<
			ReturnType<typeof this._getFormattingScore>
		>["formatting"],
	) {
		return Object.values(formatting)
			.reduce(
				(acc, curr) =>
					acc.add(new Decimal(curr.score).mul(new Decimal(curr.elementCount))),
				new Decimal(0),
			)
			.toDecimalPlaces(3)
			.toNumber();
	...
	private _calculateFormattingTotal(
		...
		if (this._readabilityConfig.enabled && readability) {
			const readabilityScore = new Decimal(readability.score)
	...
	get enabled(): boolean {
		if (
			!Value.Check(formattingEvaluatorConfigurationType, this._configuration)
		) {
	...
	async _getFormattingScore(comment: GithubCommentScore) {
		...
		this.context.logger.debug(
			"Will analyze formatting for the current content:",
			{ comment: comment.content, html },
		);
		const temp = new JSDOM(html);
		if (temp.window.document.body) {
			const res = this._classifyTagsWithWordCount(
				temp.window.document.body,
				comment,
			);
			const readability = this._calculateFleschKincaid(
				temp.window.document.body.textContent ?? "",
			);
			return { formatting: res.formatting, words: res.words, readability };
		} else {
			throw new Error(
				`Could not create DOM for comment [${JSON.stringify(comment)}]`,
			);
		}
	}
	...
	_updateTagCount(
		tagCount: Record<string, { score: number; elementCount: number }>,
		tagName: string,
		score: number,
	) {
	...
	private _addUrlsToFormatting(
		...
		urlSet.forEach(() => {
			this._updateTagCount(
				formatting,
				"a",
				this._multipliers[commentType].html["a"].score ?? 0,
			);
		});
	}
}
