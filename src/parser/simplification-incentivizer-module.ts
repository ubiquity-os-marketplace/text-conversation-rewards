...
type SimplificationRewardFile = NonNullable<
	Result[string]["simplificationReward"]
>["files"][number];
	...
	async transform(data: Readonly<IssueActivity>, result: Result) {
		...
		if (!pullRequest) {
			this.context.logger.warn(
				"This is not a pull-request, won't run SimplificationIncentivizer module.",
			);
			return result;
		}

		result = await this._processPullRequest(
		...
		try {
			...
			for (const file of files.data) {
				...
				const reward = Math.max(
					(file.deletions - file.additions) / this._simplificationRate,
					0,
				);
				if (reward !== 0) {
			...
			if (e && typeof e === "object" && "status" in e && e.status === 404) {
				this.context.logger.warn(
					"No file was found in the pull-request, skipping",
					{
						url: pull.html_url,
						owner: pull.base.repo.owner.login,
						repo: pull.base.repo.name,
