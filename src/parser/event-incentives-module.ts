...
export class EventIncentivesModule extends BaseModule {
	readonly _configuration: EventIncentivesConfiguration | null =
		this.context.config.incentives.eventIncentives;
	...
	async transform(data: Readonly<IssueActivity>, result: Result) {
		...
		const issueTimelineEvents = await octokit.paginate(
		...
		const issueReactions = await octokit.paginate(
		...
		const filteredComments = issueComments.filter(
			(comment) =>
				...
				(comment.reactions?.total_count ?? 0) > 0,
		);
		for (const comment of filteredComments) {
			const issueCommentReactions = await octokit.paginate(
		...
		for (const pull of data.linkedMergedPullRequests) {
			...
			const pullTimelineEvents = await octokit.paginate(
				octokit.rest.issues.listEventsForTimeline,
				{
			...
			for (const comment of filteredPrComments) {
				const pullCommentReactions = await octokit.paginate(
					octokit.rest.reactions.listForIssueComment,
					{
						owner: pull.self.base.repo.owner.login,
						repo: pull.self.base.repo.name,
				...
				this.processReactions(
					comment.user?.login,
					pullCommentReactions,
					result,
				);
			}

			const filteredReviewComments =
				pull.reviewComments?.filter(
			...
			for (const reviewComment of filteredReviewComments) {
				...
				const reviewReactions = await octokit.paginate(
		...
		timelineEvents.forEach((ev) => {
			if (ev.event === "reviewed" && "user" in ev && "state" in ev) {
				...
				this.increaseEventCount(
					...
					`${prefix}.received.${ev.event}`,
				);
				this.increaseEventCount(
					result,
					ev.review_requester.login,
					`${prefix}.sent.${ev.event}`,
				);
		...
		reactions.forEach((reaction) => {
			if (reaction.user) {
				this.increaseEventCount(
					result,
					reaction.user.login,
					`reaction.sent.${reaction.content}`,
				);
			}
			if (author) {
				this.increaseEventCount(
					result,
					author,
					`reaction.received.${reaction.content}`,
				);
			}
		});
	}
