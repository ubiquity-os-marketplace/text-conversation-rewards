...
export class GithubCommentModule implements Module {
  ...
  async _generateHtml(username: string, result: Result[0], stripComments = false) {
    ...
    <details>
      ...
      ${result.feeRate !== undefined ? `<h6>⚠️ ${new Decimal(result.feeRate).mul(100)}% fee rate has been applied. Consider using the <a href="https://dao.ubq.fi/dollar" target="_blank" rel="noopener">Ubiquity Dollar</a> for no fees.</h6>` : null}
