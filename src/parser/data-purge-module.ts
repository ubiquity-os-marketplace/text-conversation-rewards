...
import {
	...
	UserAssignments,
} from "../helpers/user-assigned-timespan";
...
export class DataPurgeModule extends BaseModule {
	readonly _configuration: DataPurgeConfiguration | null =
		this.context.config.incentives.dataPurge;
	_assignmentPeriods: UserAssignments = {};

	get enabled(): boolean {
		if (!this._configuration) {
			this.context.logger.warn(
				"The configuration for the module DataPurgeModule is invalid or missing, disabling.",
			);
			return false;
		}
		return true;
	...
	private _createResultComment(
		...
		return {
			...
			diffHunk: reviewComment?.pull_request_review_id
				? reviewComment?.diff_hunk
				: undefined,
		};
	}

	private async _processComment(
		comment: CommentType,
		result: Result,
	): Promise<void> {
