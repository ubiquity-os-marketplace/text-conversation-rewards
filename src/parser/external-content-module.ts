...
export class ExternalContentProcessor extends BaseModule {
	...
	private async _buildUserPrompt(
			...
			messages: [
				{
					role: "system",
					content:
						"You are an assistant that analyzes external content and provides factual summaries.",
				},
				{
					role: "user",
	...
	private async _handleExternalElements(
		...
		async function processElement(
			...
			if (!url) return;
			const llmConfig =
				configuration[isImage ? "llmImageModel" : "llmWebsiteModel"];
			...
			const altContent = await retry(
				async () => {
					...
					try {
						...
						if (!linkResponse.ok) {
							context.logger.warn(
								"Failed to fetch the content of an external element.",
								{
									url,
						...
						context.logger.warn(`The URL [${url}] could not be processed.`, {
							e,
						});
						return null;
					}
					...
					const llmResponse = await callLlm(
						{ ...prompt, reasoning_effort: llmConfig.reasoningEffort },
						context,
					);
					...
					if (typeof content !== "string" || !content.trim()) {
						throw context.logger.error(
							"Failed to generate a description for the given external element.",
							{
								url,
								contentType,
								llmResponse,
		...
		for (const data of Object.values(result)) {
			if (data.comments?.length) {
				for (const comment of data.comments.filter(
					(comment) => comment.commentType & CommentKind.ISSUE,
				)) {
