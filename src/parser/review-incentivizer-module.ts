...
export class ReviewIncentivizerModule extends BaseModule {
	...
	async transform(data: Readonly<IssueActivity>, result: Result) {
		if (!data.self?.assignees || !this.isPullRequest()) {
			this.context.logger.warn(
				"No assignees or pull request found, won't run review incentivizer module",
			);
			return result;
		}
		...
		for (const username of Object.keys(result)) {
			...
			for (const linkedPullReviews of data.linkedMergedPullRequests) {
				if (
					...
					reward.reviewRewards.push({
						reviews: reviewDiffs,
						url: linkedPullReviews.self.html_url,
					});
				}
			}
		}
	...
	async getTripleDotDiffAsObject(
		...
		const response =
			await this.context.octokit.rest.repos.compareCommitsWithBasehead({
				owner,
				repo,
		...
		for (const file of files) {
			if (
				file.status === "removed" ||
				!allowedFiles.some((o) => o.filename === file.filename)
			)
				continue;
			diff[file.filename] = {
	...
	async getReviewableDiff(
		...
		const diff = await this.getTripleDotDiffAsObject(
			owner,
			repo,
			baseSha,
			headSha,
	...
	async fetchReviewDiffRewards(
		...
		if (reviewsByUser.length == 0) {
			this.context.logger.debug("No reviews found for this pull request", {
				baseOwner,
				baseRepo,
				baseRef,
			});
		...
		const pullNumber = Number(
			reviewsByUser[0].pull_request_url.split("/").slice(-1)[0],
		);

		const prData = new PullRequestData(
			this.context,
			baseOwner,
			baseRepo,
			pullNumber,
		...
		const firstCommitSha =
			prData.pullCommits[0]?.parents?.[0]?.sha || prData.pullCommits[0]?.sha;
		if (!firstCommitSha) {
		...
		for (const [i, currentReview] of reviewsByUser.entries()) {
			...
			const baseSha = previousReview?.commit_id
				? previousReview.commit_id
				: firstCommitSha;
			...
			if (headSha && baseSha !== currentReview.commit_id) {
				try {
					...
					reviews.push({
						...
						effect: reviewEffect,
						reward:
							((reviewEffect.addition + reviewEffect.deletion) * priority) /
							this._baseRate,
						priority: priority,
					});
				} catch (e) {
					this.context.logger.error(
						`Failed to get diff between commits ${baseSha} and ${headSha}:`,
						{ e },
					);
				}
			}
	...
	get enabled(): boolean {
		if (
			!Value.Check(reviewIncentivizerConfigurationType, this._configuration)
		) {
			this.context.logger.warn(
