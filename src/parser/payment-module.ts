...
import {
	isGlobalRewardSettings,
	resolveRewardSettingsForRole,
	rewardConfigKey,
} from "../helpers/reward-settings";
import {
	BatchTransferPermit,
	ERC20_ABI,
	Erc20Wrapper,
...
export class PaymentModule extends BaseModule {
	readonly _configuration: PaymentConfiguration | null =
		this.context.config.incentives.payment;
	...
	readonly _supabase = createClient<Database>(
	...
	async transform(
		...
		const { xpUsernames, tokenGroups } =
			await this._splitUsersByRewardConfiguration(result);
		...
		if (!tokenGroups.length) {
			if (!this.context.config.rewards) {
				this.context.logger.info(
					"No permit settings found, switching to XP recording mode.",
				);
			} else {
				this.context.logger.info(
					"No reward settings matched the eligible users, switching to XP recording mode.",
				);
			}
			return result;
		}
		...
		for (const group of tokenGroups) {
			...
			await this._processTokenRewardGroup(
				data,
				groupResult,
				group.config,
				payoutMode,
	...
	private async _splitUsersByRewardConfiguration(result: Result) {
		const usernames = Object.keys(result).filter(
			(username) =>
				username !== this.context.env.PERMIT_TREASURY_GITHUB_USERNAME,
		);
		const xpUsernames: string[] = [];
		const tokenGroupMap = new Map<
			string,
			{ config: RewardSettings; usernames: string[] }
		>();
		...
		if (!rewardsConfig) {
			...
			return {
				xpUsernames,
				tokenGroups: [] as { config: RewardSettings; usernames: string[] }[],
			};
		}
	...
	private async _processTokenRewardGroup(
		...
		const privateKeyParsed = await this._parsePrivateKey(
			config.evmPrivateEncrypted,
		);
		...
		if (!isPrivateKeyAllowed) {
			this.context.logger.error(
				"Private key is not allowed to be used in this organization/repository.",
			);
			return;
		}
		...
		if (payoutMode === "transfer") {
			try {
				...
				this.context.logger.warn(
					`Failed to auto transfer rewards via batch permit transfer`,
					{ e },
				);
		...
		if (payoutMode === "permit" || directTransferError) {
			...
			for (const [username, reward] of Object.entries(result)) {
				...
				try {
					...
					this.context.logger.warn(
						`Failed to generate permits for user ${username}`,
						{ e },
					);
					continue;
				}

				result[username].permitUrl =
					`https://pay.ubq.fi?claim=${encodePermits(permits)}`;
	...
	private async _tryDirectTransfer(
		...
		if (beneficiaries.length === 0) {
			throw this.context.logger.warn(
				"Beneficiary list is empty, skipping the direct transfer of rewards...",
			);
		}

		const nonce = utils.keccak256(utils.toUtf8Bytes(issueId.toString()));
		const directTransferInfo = await this._getDirectTransferInfo(
			beneficiaries,
			config,
			privateKey,
			nonce,
		...
		this.context.logger.info(
			"Funding wallet has sufficient funds to directly transfer the rewards.",
		);
		...
		await Promise.all(
			beneficiaries.map(async (beneficiary, idx) => {
				result[beneficiary.username].explorerUrl =
					`${networkExplorer}/tx/${tx.hash}`;
				result[beneficiary.username].payoutMode = "transfer";
				await this._savePermitsToDatabase(
					result[beneficiary.username],
					{ issueUrl, issueId },
					[permits[idx]],
				);
			}),
  ...
  */
	async _getPayoutMode(
		data: Readonly<IssueActivity>,
	): Promise<PayoutMode | null> {
	...
	async _getDirectTransferInfo(
		...
		const { rewardTokenWrapper, fundingWallet } =
			await this._initializeContractsAndWallet(config, privateKey);
		const { rewardBalance, rewardAllowance, nativeBalance } =
			await this._fetchBalancesAndAllowances(rewardTokenWrapper, fundingWallet);
		...
		const hasEnoughRewardToken =
			rewardBalance.gt(totalReward) && rewardAllowance.gt(totalReward);
	...
	private async _initializeContractsAndWallet(
		config: RewardSettings,
		privateKey: string,
	) {
		...
		const fundingWallet = await getEvmWallet(
			privateKey,
	...
	private async _fetchBalancesAndAllowances(
		rewardTokenWrapper: Erc20Wrapper,
		fundingWallet: ethers.Wallet,
	) {
		const rewardBalance = await rewardTokenWrapper.getBalance(
	...
	private async _getGasEstimation(
		...
		try {
			const batchTransferPermit =
				await permit2Wrapper.generateBatchTransferPermit(
					fundingWallet,
					config.erc20RewardToken,
					transferRequests,
					BigNumber.from(nonce),
				);
			const gasEstimation = await permit2Wrapper.estimatePermitTransferFromGas(
				fundingWallet,
				batchTransferPermit,
			);
			...
			throw this.context.logger.warn(
				"Gas estimation failed for direct transfer transaction",
				{ err: error },
			);
		}
	}
	...
	async _transferReward({
		...
		rewardTokenAddress,
		networkId,
	}: DirectTransferInfo): Promise<
		[ethers.providers.TransactionResponse, PermitReward[]]
	> {
		try {
			...
			this.context.logger.info(
				`Executed permitTransferFrom contract call, transaction hash: ${tx.hash}`,
			);
			...
			const error = decodeError(e);
			throw this.context.logger.warn(
				`Direct reward transfer failed due to an EVM transaction error`,
				{ err: error },
			);
		}
	}
	...
	async _applyFees(result: Result, erc20RewardToken: string): Promise<Result> {
		...
		if (!env.PERMIT_FEE_RATE || Number(env.PERMIT_FEE_RATE) === 0) {
			this.context.logger.debug(
				"PERMIT_FEE_RATE is not set, skipping permit fee generation",
			);
			return result;
		}
		...
		if (env.PERMIT_ERC20_TOKENS_NO_FEE_WHITELIST) {
			const erc20TokensNoFee =
				env.PERMIT_ERC20_TOKENS_NO_FEE_WHITELIST.split(",");
	...
	_deductFeeFromReward(
		...
		for (const [key, rewardResult] of Object.entries(result)) {
			...
			const totalAfterFee = new Decimal(rewardResult.total)
				.mul(feeRateDecimal)
				.toNumber();
			permitFeeAmountDecimal = permitFeeAmountDecimal.add(
				new Decimal(rewardResult.total).minus(totalAfterFee),
			);
			// subtract fees
			result[key].total = Number(totalAfterFee.toFixed(2));
			result[key].feeRate = feeRateDecimal.toNumber();
			if (result[key].task) {
				result[key].task.reward = Number(
					new Decimal(result[key].task.reward).mul(feeRateDecimal).toFixed(2),
				);
			}
			if (result[key].comments) {
				for (const comment of result[key].comments) {
					if (comment.score) {
						comment.score.reward = Number(
							new Decimal(comment.score.reward).mul(feeRateDecimal).toFixed(2),
						);
					}
				}
			}
	...
	async _getOrCreateToken(address: string, network: number) {
		...
		if (!tokenData) {
			...
			if (error || !insertedToken) {
				this.context.logger.error("Failed to insert a new token:", {
					err: error,
				});
			} else {
		...
		if (!tokenId) {
			throw this.context.logger.error(
				"Failed to retrieve the related token from permit",
				{
					address,
					network,
				},
	...
	async _getOrCreatePartner(address: string) {
		...
		const { data: walletData } = await this._supabase
			.from("wallets")
			.select("id")
			.eq("address", address)
			.single();
		...
		if (!walletId) {
			throw this.context.logger.error(
				"Failed to retrieve the related wallet from permit",
				{ address },
			);
		}

		const { data: partnerData } = await this._supabase
			.from("partners")
			.select("id")
		...
		if (!partnerData) {
			...
			if (error || !insertedPartner) {
				this.context.logger.error("Failed to insert a new partner:", {
					err: error,
				});
			} else {
		...
		if (!partnerId) {
			throw this.context.logger.error(
				"Failed to retrieve the related partner from permit",
				{ address },
			);
		}
	...
	private async _tryPersistPermit({
		...
		if (!amount.gt(0)) {
			this.context.logger.warn(
				"Skipping permit persistence because amount is zero.",
				{
		...
		try {
			const { data: userData } = await this._supabase
				.from("users")
				.select("id")
				.eq("id", userId)
				.single();
			...
			const locationId =
				await this.context.adapters.supabase.location.getOrCreateIssueLocation(
					issue,
				);
			const tokenId = await this._getOrCreateToken(
				permit.tokenAddress,
				permit.networkId,
			);
	 ...
	 */
	private async _upsertPermitRecord(
		...
		if (!fallbackReason) {
			this.context.logger.error("Failed to upsert permit via RPC", {
				err: error,
			});
			return false;
		}
	...
	private _getMissingPermitMetadata(
		insertData: Database["public"]["Tables"]["permits"]["Insert"],
	): string[] {
		...
		if (
			insertData.permit2_address === null ||
			insertData.permit2_address === undefined
		) {
	...
	private _getRpcFallbackReason(
		...
		const isSchemaCacheMissing =
			...
			normalizedMessage.includes("upsert_permit_max");
		const isRpcMissing =
			code === "42883" ||
			code === "PGRST202" ||
			normalizedMessage.includes("upsert_permit_max does not exist") ||
			isSchemaCacheMissing;
		const isRpcPermissionDenied =
			code === "42501" || normalizedMessage.includes("permission denied");
	...
	private async _insertPermitWithFallback(
		...
		if (!this._isUniqueViolation(insertError)) {
			this.context.logger.error("Failed to insert permit after RPC fallback", {
				err: insertError,
			});
			return false;
		}
	...
	private async _tryUpdateExistingPermitAfterFallback(
		...
		const updateResult = await this._attemptPermitUpdate(
			existingPermit,
			insertData,
		);
	...
	private async _handlePermitUpdateRace(
		...
		const retryResult = await this._attemptPermitUpdate(
			refreshedPermit,
			insertData,
		);
	...
	async _parsePrivateKey(evmPrivateEncrypted: string) {
		const privateKeyDecrypted = await decrypt(
			evmPrivateEncrypted,
			String(this.context.env.X25519_PRIVATE_KEY),
		);
		return parseDecryptedPrivateKey(privateKeyDecrypted);
	}

	private async _saveXpRecord(
		userId: number,
		issue: { issueId: number; issueUrl: string },
		...
		if (userError || !userData) {
			this.context.logger.info(
				`User with ID ${userId} not found in database. Attempting to create new user.`,
			);
			...
			if (createError || !newUserData) {
				throw this.context.logger.error(
					`Failed to create user with ID ${userId} in database.`,
					{ createError },
				);
			}

			this.context.logger.ok(
				`Successfully created user with ID ${userId} in database.`,
			);
		...
		const amountString = new Decimal(numericAmount)
			.mul(new Decimal(10).pow(18))
			.toFixed();

		const { data: existingXp, error: duplicateCheckError } =
		...
		if (duplicateCheckError) {
			throw this.context.logger.error(
				"Error checking for duplicate XP records",
				{ duplicateCheckError },
			);
		}
		...
		if (existingXp) {
			...
			if (updateError) {
				throw this.context.logger.error(
					"Failed to update XP record in database",
					{ updateError },
				);
			}
			this.context.logger.ok(
				`XP record updated successfully for userId: ${userId}, issueId: ${issue.issueId}`,
			);
		} else {
			const insertData: Database["public"]["Tables"]["permits"]["Insert"] = {
				amount: amountString,
				...
				token_id: null,
				nonce: BigInt(
					utils.keccak256(utils.toUtf8Bytes(`${userId}-${issue.issueId}`)),
				).toString(),
				deadline: "0",
			...
			if (insertError) {
				throw this.context.logger.error(
					"Failed to insert XP record into database",
					{ insertError },
				);
			}
			this.context.logger.ok(
				`XP record inserted successfully for userId: ${userId}, issueId: ${issue.issueId}`,
			);
		}
	}
	...
	async _handleXpRecording(result: Result): Promise<Result> {
		...
		if (!issueId) {
			this.context.logger.error(
				"[PaymentModule] Could not extract issue ID from URL for XP recording.",
				{
					issueUrl,
				},
			);
		...
		for (const [key, value] of Object.entries(result)) {
			if (key === this.context.env.PERMIT_TREASURY_GITHUB_USERNAME) {
				this.context.logger.debug(
					`Skipping XP recording for treasury user ${key}.`,
				);
				continue;
			}
			if (!value.userId) {
				this.context.logger.warn(
					`[PaymentModule] Missing userId for user ${key}, cannot record XP.`,
				);
				continue;
			}
			...
			try {
				...
				this.context.logger.ok(
					`Successfully recorded XP for user ${key} (ID: ${value.userId})`,
				);
			} catch (e) {
				this.context.logger.error(
					`[PaymentModule] Failed to record XP for user ${key}`,
					{ e },
				);
			}
		}
	...
	async _isPrivateKeyAllowed(
		...
		if (
			privateKeyParsed.allowedOrganizationId &&
			!privateKeyParsed.allowedRepositoryId
		) {
