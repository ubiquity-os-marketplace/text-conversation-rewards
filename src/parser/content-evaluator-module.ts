...
import {
	...
	CommentKind,
	CommentType,
} from "../configuration/comment-types";
...
export class ContentEvaluatorModule extends BaseModule {
	readonly _configuration: ContentEvaluatorConfiguration | null =
		this.context.config.incentives.contentEvaluator;
	...
	constructor(context: ContextPlugin) {
		...
		if (this._configuration?.multipliers) {
			this._fixedRelevances = this._configuration.multipliers.reduce(
				(acc, curr) => {
					return {
						...acc,
	...
	async transform(data: Readonly<IssueActivity>, result: Result) {
		if (!this._configuration?.openAi.tokenCountLimit) {
			throw this.context.logger.fatal(
				"Token count limit is missing, comments cannot be evaluated.",
			);
		}
			...
			if (specificationBody && comments.length) {
				promises.push(
					this._processComment(
						...
						(commentsWithScore) =>
							(currentElement.comments = commentsWithScore),
					),
				);
			}
		}
	...
	private async _handleRewardsForOriginalAuthor(body: string, result: Result) {
		...
		if (
			!originalComment ||
			!issueAuthor ||
			issueAuthor === originalComment.username
		)
			return;
		...
		await this._addRewardForAuthor(
			result,
	...
	private _extractAndAdjustSpecReward(result: Result) {
		for (const resultKey of Object.keys(result)) {
			...
			if (spec && spec.score?.reward !== undefined) {
				...
				const originalAuthorReward = reward
					.mul(this._originalAuthorWeight)
					.toNumber();
	...
	private async _addRewardForAuthor(
		result: Result,
		username: string,
		specReward: GithubCommentScore,
	) {
	...
	private async _fetchGithubUserId(username: string) {
		try {
			const userResponse = await this.context.octokit.rest.users.getByUsername({
				username,
			});
			...
			this.context.logger.warn("Failed to fetch the user ID.", {
				username,
				err,
			});
			return 0;
	...
	async _processComment(
		...
		const { commentsToEvaluate, prCommentsToEvaluate } =
		...
		const relevancesByAi = await retry(
			async () => {
				...
				const expectedRelevances = this.isPullRequest()
					? prCommentsToEvaluate.length
					: commentsToEvaluate.length;
				...
				onError: async (error) => {
					if (this.context.config.incentives.githubComment?.post) {
						await this.context.commentHandler.postComment(
							this.context,
							this.context.logger.ok("Results are being retried", {
								err: error,
							}),
							{
								updateComment: true,
		...
		for (const currentComment of commentsWithScore) {
			...
			const priority =
				...
				!(currentComment.commentType & CommentAssociation.SPECIFICATION)
					? basePriority
					: 1;
	...
	_splitArrayToChunks<T extends CommentToEvaluate[] | AllComments>(
		array: T,
		chunks: number,
	) {
	...
	async _splitPromptForIssueCommentEvaluation(
		...
		for (let currentChunk = 2; currentChunk <= maxChunks; currentChunk++) {
			...
			const maxPromptTokens = Math.max(
				...chunkTokenEstimates.map((estimate) => estimate.promptTokens),
			);
			const maxOutputTokens = Math.max(
				...chunkTokenEstimates.map((estimate) => estimate.outputTokens),
			);
		...
		if (!chunks) {
			const fallbackPrompt = this._generatePromptForComments(
				specification,
				username,
				allComments,
			);
			const dummyResponse = JSON.stringify(
				this._generateDummyResponse(comments),
				null,
				2,
			);
			const fallbackPromptTokens = this._calculateMaxTokens(
				fallbackPrompt,
				Infinity,
			);
			const fallbackOutputTokens = this._calculateMaxTokens(dummyResponse);
			this.context.logger.warn(
				"Unable to reduce issue comment evaluation below token limit",
				{
		...
		for (const commentSplit of this._splitArrayToChunks(allComments, chunks)) {
			const targetComments = commentSplit.filter(
			...
			const dummyResponse = JSON.stringify(
				this._generateDummyResponse(targetComments),
				null,
				2,
			);
			const maxOutputTokens = this._calculateMaxTokens(dummyResponse);
			const promptForComments = this._generatePromptForComments(
				specification,
				username,
				commentSplit,
			);
			...
			for (const [key, value] of Object.entries(
				await this._submitPrompt(promptForComments, maxOutputTokens),
			)) {
		...
		for (const key of Object.keys(commentRelevances)) {
			...
			commentRelevances[key] = new Decimal(commentRelevances[key])
				.div(count)
				.toNumber();
		}
	...
	private _getIssueChunkEstimates(
		...
		return this._splitArrayToChunks(allComments, chunks)
			.map((chunk) => {
				...
				const dummyResponse = JSON.stringify(
					this._generateDummyResponse(
						chunk.filter((comment) => comment.author === username),
					),
					null,
					2,
				);
			...
			.filter(
				(value): value is { promptTokens: number; outputTokens: number } =>
					value !== null,
			);
	}

	async _splitPromptForPullRequestCommentEvaluation(
		specification: string | string[],
		comments: PrCommentToEvaluate[],
	) {
		const commentRelevances: Relevances = {};
		...
		while (
			Math.max(
				...this._splitArrayToChunks(comments, chunks).map(
					(chunk) =>
						this._calculateMaxTokens(
							JSON.stringify(this._generateDummyResponse(chunk), null, 2),
						) +
						this._calculateMaxTokens(
		...
		for (const commentSplit of this._splitArrayToChunks(comments, chunks)) {
			const dummyResponse = JSON.stringify(
				this._generateDummyResponse(commentSplit),
				null,
				2,
			);
			const maxOutputTokens = this._calculateMaxTokens(dummyResponse);
			const promptForComments = this._generatePromptForPrComments(
				specification,
				commentSplit,
			);
	...
	async _evaluateComments(
		...
		if (userIssueComments.length && !this.isPullRequest()) {
			const dummyResponse = JSON.stringify(
				this._generateDummyResponse(userIssueComments),
				null,
				2,
			);
			const maxOutputTokens = this._calculateMaxTokens(dummyResponse);

			const promptForIssueComments = this._generatePromptForComments(
				specification,
				username,
				allComments,
			);
			if (
				this._calculateMaxTokens(promptForIssueComments, Infinity) +
					maxOutputTokens >
				this._tokenLimit
			) {
		...
		if (userPrComments.length && this.isPullRequest()) {
			...
			const prSpecifications: string | string[] = closingIssueBodies.length
				? closingIssueBodies
				: specification;
			const dummyResponse = JSON.stringify(
				this._generateDummyResponse(userPrComments),
				null,
				2,
			);
			const maxOutputTokens = this._calculateMaxTokens(dummyResponse);

			const promptForPrComments = this._generatePromptForPrComments(
				prSpecifications,
				userPrComments,
			);
			if (
				this._calculateMaxTokens(promptForPrComments, Infinity) +
					maxOutputTokens >
				this._tokenLimit
			) {
				prCommentRelevances =
					await this._splitPromptForPullRequestCommentEvaluation(
						prSpecifications,
						userPrComments,
					);
			} else {
				prCommentRelevances = await this._submitPrompt(
					promptForPrComments,
					maxOutputTokens,
				);
			}
	...
	private async _getClosingIssueBodies(): Promise<string[]> {
		try {
			...
			const linked =
				await this.context.octokit.graphql.paginate<PullRequestClosingIssue>(
					LINKED_ISSUES,
					{
						owner,
						repo,
			...
			const edges =
				linked.repository.pullRequest.closingIssuesReferences.edges ?? [];
			const bodies: string[] = [];
			...
			throw this.context.logger.warn(
				"Failed to collect the closing issue's body for a linked issue.",
				{ e },
			);
		}
	}
	...
	async _submitPrompt(prompt: string, maxTokens: number): Promise<Relevances> {
		try {
			...
			if (typeof answer !== "string" || !answer.trim()) {
				throw this.context.logger.error(
					"Unexpected response format: Expected JSON string in message content",
				);
			}
			...
			try {
				...
				try {
					...
					this.context.logger.debug(
						`LLM extracted JSON (using max_tokens: ${maxTokens}): ${extracted}`,
					);
					parsedJson = JSON.parse(extracted);
				} catch (e2) {
					parseErrors.push(e2);
					throw this.context.logger.error(
						"Failed to parse a JSON object from the LLM response",
						{
							maxTokens,
							parseErrors,
			...
			const relevances = Value.Decode(
				openAiRelevanceResponseSchema,
				parsedJson,
			);
			this.context.logger.debug(
				`Relevances by the LLM: ${JSON.stringify(relevances)}`,
			);
			return relevances;
		} catch (e) {
			this.context.logger.error(
				`Invalid response type received from the LLM while evaluating: \n\n${e}`,
				{
					error: e as Error,
				},
			);
