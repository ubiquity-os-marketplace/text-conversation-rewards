...
export class UserExtractorModule extends BaseModule {
	private readonly _configuration: UserExtractorConfiguration | null =
		this.context.config.incentives.userExtractor;
	...
	async transform(
		...
		const closedEvents =
			data.events?.filter((event) => event.event === "closed") ?? [];
		...
		if (closedEvents.length > 0) {
			...
			taskUrl = this._addEventAnchorToHtmlUrl(
				`${data.self?.html_url}`,
				closedEvents[0].id.toString(),
			);
		} else {
		...
		for (const review of data.linkedMergedPullRequests) {
			...
			review.self?.requested_reviewers?.forEach(
