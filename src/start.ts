...
export async function getIssueEvents(
	...
	return await octokit.paginate(
		octokit.rest.issues.listEvents.endpoint.merge(issueParams),
	);
}
...
export async function getPullRequestReviews(
	...
	const { octokit } = context;
	return await octokit.paginate(
		octokit.rest.pulls.listReviews.endpoint.merge(pullParams),
	);
}
...
export async function getPullRequestReviewComments(
	...
	const { octokit } = context;
	return await octokit.paginate(
		octokit.rest.pulls.listReviewComments.endpoint.merge(pullParams),
	);
}
