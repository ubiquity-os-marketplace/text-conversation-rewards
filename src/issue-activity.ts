...
export class IssueActivity {
	...
	private async _getLinkedIssues(): Promise<ClosedByPullRequestsReferences[]> {
		...
		const linked =
			await this._context.octokit.graphql.paginate<PullRequestClosingIssue>(
				LINKED_ISSUES,
				{
					owner,
					repo,
	...
	private async _getLinkedPullRequests(): Promise<PullRequest[]> {
		this._context.logger.info(
			"Trying to fetch linked pull-requests for",
			this._issueParams,
		);
		...
		if (isPullRequestEvent(this._context)) {
			...
			this._context.payload.issue.pull_request?.html_url
		) {
			...
			const issueAssigneeLogins =
				"issue" in this._context.payload
					...
					: [];
			const linkedPulls: LinkedPullRequest[] = await collectLinkedPulls(
				this._context,
				this._issueParams,
			);
			const linkedPullUrls = linkedPulls
				.filter((pullRequest) => {
					...
					const specialUserGroups =
						this._context.config.incentives.specialUsers ?? [];
		...
		this._context.logger.info(
			`Collected linked pull-requests: ${pulls.join(", ")}`,
		);
	...
	private async _processSingleLinkedPullRequest(
	...
	private async _tryAddLinkedComment(
		comment: AugmentedComment,
		comments: Array<AugmentedComment>,
	) {
		if (!isPullRequest(this._context)) {
			if (
	...
	async _getLinkedPullRequestComments() {
		const commentPromises = this.linkedMergedPullRequests.map(
			(linkedPullRequest) =>
...
export class PullRequest {
	...
	async init() {
		[this.self, this.reviews, this.reviewComments, this.comments] =
			await Promise.all([
