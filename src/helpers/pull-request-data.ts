...
type CommitParent = NonNullable<
	CommitEdge["node"]["commit"]["parents"]["nodes"]
>[number];
...
export class PullRequestData {
	...
	public async fetchData() {
		const commitsData =
			await this._context.octokit.graphql.paginate<PullRequestCommitsQuery>(
		...
		const commitEdges: CommitEdge[] =
			commitsData?.repository?.pullRequest?.commits?.edges ?? [];
		this._pullCommits = commitEdges
			.map(
				(edge: CommitEdge): LightweightCommit => ({
					...
					parents: edge.node.commit.parents.nodes?.map(
						(parent: CommitParent) => ({ sha: parent.oid }),
					),
				}),
			)
