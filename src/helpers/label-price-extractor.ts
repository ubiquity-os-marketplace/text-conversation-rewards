 ...
 */
export async function getTaskReward(
	...
	if (issue) {
		if (issue.pull_request) {
			const linkedIssues =
				await context.octokit.graphql.paginate<PullRequestClosingIssue>(
					LINKED_ISSUES,
					{
			...
			const perIssueMinimums =
				linkedIssues.repository.pullRequest.closingIssuesReferences.edges
					.map((edge: ClosingIssueEdge) =>
						getSortedPrices(edge.node.labels?.nodes),
					)
