...
const rewardRoleCache = new WeakMap<
	ContextPlugin,
	Map<string, RewardUserRole>
>();
...
async function fetchRepoPermissionInfo(
	...
	try {
		...
		context.logger.warn(
			`Failed to determine collaborator permission level for ${username}`,
			{ e },
		);
		return {
			role: null,
...
function membershipRoleToRewardRole(
	role: string | null,
): RewardUserRole | null {
	if (!role) {
		return null;
...
export async function getUserRewardRole(
	context: ContextPlugin,
	username: string,
): Promise<RewardUserRole> {
...
export async function isUserAllowedToGenerateRewards(
	context: ContextPlugin,
	login?: string,
): Promise<boolean> {
