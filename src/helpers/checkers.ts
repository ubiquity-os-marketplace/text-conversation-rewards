...
type RequestedReviewer = NonNullable<
	GitHubPullRequest["requested_reviewers"]
>[number];
type ReviewAuthorAssociation = NonNullable<
	GitHubPullRequestReviewState["author_association"]
>;

const COLLABORATOR_REVIEW_ASSOCIATIONS: ReviewAuthorAssociation[] = [
	"COLLABORATOR",
	"MEMBER",
	"OWNER",
];
	...
	if (data.self.closed_by.id === issueCreator.id) {
		const pricingEventsByNonAssignee = data.events.find(
			(event) =>
				event.event === "labeled" &&
				"label" in event &&
				(event.label.name.startsWith("Time: ") ||
					event.label.name.startsWith("Priority: ")) &&
...
export function nonAssigneeApprovedReviews(data: Readonly<IssueActivity>) {
	...
	if (data.self?.pull_request) {
		const pullRequestAuthorId =
			linkedPullRequest.self?.user?.id ?? data.self.user?.id;
		const excludedAuthorIds = new Set(
			[
				pullRequestAuthorId,
				...data.linkedIssues.map((issue) => issue.node.author?.id),
			]
				.filter(Boolean)
				.map(String),
		);
		return hasApprovedReviewByCollaborator(
			linkedPullRequest.reviews,
			excludedAuthorIds,
		);
	}
	...
	return hasIssueContextApprovedReview(
...
function isReviewRequestedForUser(
	pullRequest: GitHubPullRequest,
	...
	return (
		pullRequest.requested_reviewers?.some(
			(reviewer: RequestedReviewer) => reviewer.id === review.user?.id,
		) ?? false
	);
}

function isReviewByCollaborator(review: GitHubPullRequestReviewState) {
	return COLLABORATOR_REVIEW_ASSOCIATIONS.includes(
		review.author_association as ReviewAuthorAssociation,
	);
}
...
export async function isAdmin(
	...
	try {
		...
		context.logger.debug(
			`Retrieved collaborator permission level for ${username}.`,
			{
				username,
				owner: context.payload.repository.owner.login,
				repo: context.payload.repository.name,
		...
		return (
			userPerms.data.role === "admin" ||
			userPerms.data.role === "billing_manager"
		);
	} catch (e) {
		context.logger.debug(
			`${username} is not a member of ${context.payload.repository.owner.login}`,
			{ e },
		);
		return false;
	}
