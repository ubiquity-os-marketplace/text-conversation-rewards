...
export async function getContract(
	...
	const provider = new ethers.providers.JsonRpcProvider(
		`https://permit2-rpc-proxy.deno.dev/${networkId}`,
	);
...
export class Permit2Wrapper {
	...
	async generateBatchTransferPermit(
		...
		const permitBatchTransferFromData: PermitBatchTransferFrom = {
			permitted: transferRequests.map((request) => ({
				token: tokenAddress,
	 ...
	 * @returns Gas estimation of permitTransferFrom contract call
	 */
	async estimatePermitTransferFromGas(
		evmWallet: Wallet,
		batchTransferPermit: BatchTransferPermit,
	): Promise<BigNumber> {
		const gasLimit = await this._permit2
			.connect(evmWallet)
			.estimateGas.permitTransferFrom(
				{
