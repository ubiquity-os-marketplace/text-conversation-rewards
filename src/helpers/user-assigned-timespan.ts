...
type IssueEvent = Awaited<
	ReturnType<ContextPlugin["octokit"]["rest"]["issues"]["listEvents"]>
>["data"][number];
 ...
 */
export async function getAssignmentPeriods(
	...
	events
		...
		.forEach((event: IssueEvent) => {
			...
			const lastPeriod =
				userAssignments[username][userAssignments[username].length - 1];
