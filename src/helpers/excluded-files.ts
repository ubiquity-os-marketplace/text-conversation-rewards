...
function matchesGitPattern(filePath: string, pattern: string): boolean {
	...
	if (pattern.includes("/")) {
		...
		return (
			minimatch(basename(filePath), pattern) ||
...
export async function getExcludedFiles(
	...
	const [gitAttributesContent, prettierIgnoreContent, tsConfigContent] =
		await Promise.all([
...
async function getFileContent(
	...
	try {
		...
		if (err instanceof Error && "status" in err && err.status === 404) {
			context.logger.debug(`[${path}] was not found for ${owner}/${repo}`, {
				err,
			});
			return null;
		}
		throw context.logger.error(
			`Could not fetch the list of files to be excluded.`,
			{
				err,
			},
		);
