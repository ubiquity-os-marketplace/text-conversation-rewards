...
export async function manuallyCloseIssue(
	context: ContextPlugin<"issue_comment.created">,
) {
 ...
 */
export async function checkIfClosedByCommand(
	...
	try {
		...
		if (
			lastReopenedEvent &&
			(!("created_at" in lastReopenedEvent) ||
				!("created_at" in lastClosedEvent))
		) {
			return false;
		}

		const lastReopenedTime = lastReopenedEvent
			? new Date(lastReopenedEvent.created_at)
			: null;
		...
		const commandComments = allComments.filter((comment) => {
			...
			const isAfterLastReopened = lastReopenedTime
				? commentTime > lastReopenedTime
				: true;
			const isBeforeClose = commentTime < closeTime;
			return (
				isAfterLastReopened &&
				isBeforeClose &&
				comment.body.trim().startsWith("/finish")
			);
		});
