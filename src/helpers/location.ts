...
export class Location extends Super {
	 ...
	 */
	public async upsert(
		locationData: Database["public"]["Tables"]["locations"]["Insert"],
	) {
		if (this.locationId) {
			...
			if (error || !data) {
				throw this.context.logger.error("Failed to update location", {
					err: error,
					locationData,
				});
			}
			...
			if (error || !data) {
				throw this.context.logger.error("Failed to insert location", {
					err: error,
					locationData,
				});
			}
	...
	public async getOrCreateIssueLocation(issue: {
		issueId: number;
		issueUrl: string;
	}) {
		...
		if (!locationData) {
			const issueItem = await getRepo(
				this.context,
				parseGitHubUrl(issue.issueUrl),
			);
			...
			if (!newLocationData || error) {
				this.context.logger.error("Failed to create a new location", {
					err: error,
				});
			} else {
		...
		if (!locationId) {
			throw this.context.logger.error(
				"Failed to retrieve the related location from issue",
				{ issue },
			);
		}
		return locationId;
