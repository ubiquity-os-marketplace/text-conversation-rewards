...
describe.each(automaticTransferModeVector)(
	...
	(automaticTransferMode) => {
		...
		beforeAll(async () => {
			const { isAdmin, isCollaborative } = await import(
				"../src/helpers/checkers"
			);

			isAdminMocked = isAdmin as jest.Mock;
			isCollaborativeMocked = isCollaborative as jest.Mock;
			ctx.config.incentives.payment = {
				automaticTransferMode: automaticTransferMode,
			};
			paymentResults = { ...originalpaymentResults };
			const payoutMode: PayoutMode = automaticTransferMode
				? "transfer"
				: "permit";

			for (const username of Object.keys(paymentResults)) {
				if (!paymentResults[username]["permitUrl"] && !automaticTransferMode)
					continue; // getWalletByUserId mock returns null here
