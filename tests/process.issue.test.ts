...
const issueUrl =
	process.env.TEST_ISSUE_URL ??
	"https://github.com/ubiquity-os/conversation-rewards/issues/5";
...
function getExternalContentProcessor(context: ContextPlugin) {
	...
	Reflect.set(instance, "_llmWebsite", {
		chat: { completions: { create: jest.fn() } },
	});
	Reflect.set(instance, "_llmImage", {
		chat: { completions: { create: jest.fn() } },
	});
	return instance;
}
...
beforeAll(async () => {
	...
	({ ContentEvaluatorModule } = await import(
		"../src/parser/content-evaluator-module"
	));
	({ DataPurgeModule } = await import("../src/parser/data-purge-module"));
	({ FormattingEvaluatorModule } = await import(
		"../src/parser/formatting-evaluator-module"
	));
	({ GithubCommentModule } = await import(
	...
	({ UserExtractorModule } = await import(
		"../src/parser/user-extractor-module"
	));
	({ ReviewIncentivizerModule } = await import(
		"../src/parser/review-incentivizer-module"
	));
	({ EventIncentivesModule } = await import(
		"../src/parser/event-incentives-module"
	));
	({ SimplificationIncentivizerModule } = await import(
		"../src/parser/simplification-incentivizer-module"
	));
	({ ExternalContentProcessor } = await import(
		"../src/parser/external-content-module"
	));
...
describe("Modules tests", () => {
	...
	beforeEach(async () => {
		...
		jest
			.spyOn(ctx.octokit.rest.repos, "compareCommits")
			.mockImplementation(async () => {
				return {
					data: {
						files: [
					...
					Awaited<typeof ctx.octokit.rest.repos.compareCommits>
				>;
			});
	});
	...
	it("Should incentivize simplifications", async () => {
		jest
			...
			.mockImplementation(
				(
					...
					const relevance: { [k: string]: number } = {};
					(prCommentsToEvaluate ?? commentsToEvaluate ?? []).forEach(
						(comment) => {
							relevance[`${comment.id}`] = 0.8;
						},
					);
		...
		const ctx = {
			...
			payload: {
				pull_request: {
					html_url:
						"https://github.com/ubiquity-os/conversation-rewards/pull/12",
					number: 1,
					state_reason: "completed",
					assignees: [
						{
	...
	it("Should return 0 when priceTagReward is 0 (due to Price: 0 label)", async () => {
		...
		const mockIssueWithZeroPrice = {
			labels: [{ name: "Price: 0 USD" }],
		} as unknown as GitHubIssue;

		const rewardLimit = await processor._getRewardsLimit(
			mockIssueWithZeroPrice,
		);
	...
	it("Should return the max corresponding to the label of the issue if enabled", async () => {
		...
		expect(total).toMatchObject({
			user1: {
				total: 9.25,
				task: { multiplier: 0.5, reward: 18.5 },
				userId: 0,
			},
	...
	it("It should warn the user if wallet is not set", async () => {
		...
		await processor.run(activity);
		const result = JSON.parse(processor.dump());
		expect(result["whilefoo"].evaluationCommentHtml).toContain(
			"Wallet address is not set",
		);
	}, 120000);
		...
		await expect(
			retry(
				async () => {
					called += 1;
					if (called === 1)
						throw Object.assign(new Error("LLM API error: 500"), {
							status: 500,
						});
