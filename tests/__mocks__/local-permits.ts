...
} from "@ubiquity-os/permit-generation/utils";
...
async function getPrivateKey(evmPrivateEncrypted: string) {
	try {
		const privateKeyDecrypted = await decrypt(
			evmPrivateEncrypted,
			String(process.env.X25519_PRIVATE_KEY),
		);
...
export async function generatePermitUrlPayload(
	...
	const { data: userData } = await context.octokit.rest.users.getByUsername({
		username,
	});
	...
	const nonce = BigInt(
		utils.keccak256(utils.toUtf8Bytes(`${userData.id}-${nodeId}`)),
	);
	const walletAddress = await adapters.supabase.wallet.getWalletByUserId(
		userData.id,
	);
