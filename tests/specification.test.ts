...
const issueUrl =
	"https://github.com/ubiquity-os/conversation-rewards/issues/71";
...
beforeAll(async () => {
	...
	jest
		...
		.mockImplementation(
			(specificationBody, userId, comments, allComments, prComments) => {
...
describe("Content Evaluator Module Test", () => {
	...
	it("Should handle comments with original author references", async () => {
		...
		const originalAuthorComment = {
			...
			html_url:
				"https://github.com/ubiquity-os/conversation-rewards/issues/71#issuecomment-1571243461",
		...
		expect(extractedInfo?.url).toBe(
			"https://github.com/ubiquity/work.ubq.fi/issues/69#issuecomment-1234",
		);
		...
		expect(result["reviewer"].comments).toEqual(
			expect.arrayContaining([
				expect.objectContaining({ id: activity.self?.id }),
			]),
		);
