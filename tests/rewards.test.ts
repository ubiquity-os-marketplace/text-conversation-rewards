...
describe("Rewards tests", () => {
	...
	it("Should distribute rewards based on authorship percentages in issue body edits", async () => {
		...
		if ("issue" in modifiedCtx.payload) {
			...
			modifiedCtx.payload.issue.assignees = [
				author,
			] as typeof modifiedCtx.payload.issue.assignees;
		}
		...
		expect(fs.readFileSync("./output.html", "utf-8")).toEqual(
			fs.readFileSync(
				"./tests/__mocks__/results/output-authorship-reward.html",
				"utf-8",
			),
		);
	}, 120000);
	...
	it("Should fall back to XP when no matching role rewards are defined", async () => {
		...
		server.use(
			http.get("https://api.github.com/orgs/:org/memberships/:username", () =>
		...
		expect(Object.values(result)?.[0].evaluationCommentHtml).not.toMatch(
	...
	it("Should display a capped messaged on capped rewards", async () => {
		...
		if (labels) {
			const idx = labels.findIndex(
				(item: NonNullable<GitHubIssue["labels"]>[number]) => {
