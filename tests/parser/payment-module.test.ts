...
jest
	.spyOn(ReviewIncentivizerModule.prototype, "getTripleDotDiffAsObject")
...
describe("payment-module.ts", () => {
	describe("applyFees()", () => {
		...
		it("Should apply fees", async () => {
			const paymentModule = new PaymentModule(ctx);
			const resultAfterFees = await paymentModule._applyFees(
				getResultOriginal(),
				WXDAI_ADDRESS,
			);
			...
			expect(
				resultAfterFees["molecula451"].comments?.[0].score?.reward,
			).toEqual(9);
			expect(resultAfterFees["0x4007"].total).toEqual(10);
			expect(resultAfterFees["0x4007"].task?.reward).toEqual(8.99);
			expect(resultAfterFees["0x4007"].comments?.[0].score?.reward).toEqual(
				1.01,
			);
	...
	describe("_getBeneficiaries()", () => {
		...
		it("Should return the correct total payable amount", async () => {
			const paymentModule = new PaymentModule(ctx);
			const beneficiaries = await paymentModule._getBeneficiaries(
				getResultOriginal(),
			);
	...
	describe("_savePermitsToDatabase()", () => {
		...
		it("should skip inserting permits with zero amount", async () => {
			...
			(
				paymentModule as unknown as { _supabase: typeof supabaseMock }
			)._supabase = supabaseMock;
			jest
				.spyOn(
					paymentModule as unknown as {
						_getOrCreateToken: () => Promise<number>;
					},
					"_getOrCreateToken",
				)
			...
			jest
				.spyOn(
					paymentModule as unknown as {
						_getOrCreatePartner: () => Promise<number>;
					},
					"_getOrCreatePartner",
				)
			...
			await paymentModule._savePermitsToDatabase(
				rewardResult,
				{ issueId: 99, issueUrl: "https://example.com" },
				[zeroAmountPermit],
			);
	...
	describe("_getPayoutMode()", () => {
		...
		it("Should return null if the `payoutMode` was already set to `direct`", async () => {
			...
			let payoutMode = await paymentModule._getPayoutMode({
				comments: [
					{ body: `...${PAYOUT_MODE_TRANSFER}....`, user: { type: "Bot" } },
				],
		...
		it("Should return `permit` if the `payoutMode` was already set to `permit` or `autoTransferMode` is set to `false`", async () => {
			...
			let payoutMode = await paymentModule._getPayoutMode({
				comments: [
					{ body: `...${PAYOUT_MODE_PERMIT}...`, user: { type: "Bot" } },
				],
			} as unknown as IssueActivity);
	...
	const fundingWalletPrivateKey =
		"ac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80";
	describe("_canTransferDirectly()", () => {
		beforeEach(() => {
			ctx.env.PERMIT_FEE_RATE = EMPTY_STRING;
			ctx.env.X25519_PRIVATE_KEY =
				"wrQ9wTI1bwdAHbxk2dfsvoK1yRwDc0CEenmMXFvGYgY";
		});
		...
		it("Should return a transferInfo when the funding wallet has enough reward tokens and gas", async () => {
			...
			const logCallMetadata = spyConsoleLog.mock.calls.map(
				(call) => call[1],
			)[0] as {
			...
			expect(logCallMetadata.gas.required).toEqual(
				parseUnits("0.02", 18).toString(),
			);
			expect(logCallMetadata.rewardToken.has).toEqual(
				parseUnits("20000", 18).toString(),
			);
			expect(logCallMetadata.rewardToken.allowed).toEqual(
				parseUnits("20000", 18).toString(),
			);
			expect(logCallMetadata.rewardToken.required).toEqual(
				parseUnits("111.11", 18).toString(),
			);
		...
		it("Should reject if the funding wallet has enough reward tokens but insufficient gas", async () => {
			...
			const rewardSettings = ctx.config.rewards as RewardSettings;
			await expect(
				paymentModule._getDirectTransferInfo(
					beneficiaries,
					rewardSettings,
					fundingWalletPrivateKey,
					"0",
		...
		it("Should reject if the funding wallet has insufficient reward tokens", async () => {
			...
			web3Mocks.Erc20Wrapper.getBalance.mockReturnValueOnce(
				parseUnits("50", 18),
			);
	...
	describe("_isPrivateKeyAllowed()", () => {
		...
		it("Should return false if private key could not be decrypted", async () => {
			...
			const privateKeyEncrypted =
				"Y-29-JttQ7xNBMOQylST_9kgVjVKnvkYlyihbqsBwGDmuBi7ZlaIh1I6cTDzrMiR";
			const githubContextOrganizationId = 1;
			const githubContextRepositoryId = 2;

			const privateKeyParsed =
				await paymentModule._parsePrivateKey(privateKeyEncrypted);
			...
			const logCallArgs = spyConsoleLog.mock.calls.map((call) => call[0]);
			expect(logCallArgs[0]).toMatch(
				new RegExp(`.*Private key could not be decrypted`),
			);
			spyConsoleLog.mockReset();
		});
