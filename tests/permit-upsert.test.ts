...
type UpsertModule = {
	_upsertPermitRecord: (
		data: Database["public"]["Tables"]["permits"]["Insert"],
	) => Promise<boolean>;
};
...
describe("PaymentModule _upsertPermitRecord", () => {
	...
	it("returns true when RPC succeeds", async () => {
		...
		const didUpsert = await (
			paymentModule as unknown as UpsertModule
		)._upsertPermitRecord(baseInsertData);
		expect(didUpsert).toBe(true);
		expect(mockRpc).toHaveBeenCalledTimes(1);
		const [rpcName, rpcArgs] = mockRpc.mock.calls[0] as unknown as [
			string,
	...
	it("falls back to insert when RPC is missing", async () => {
		mockRpc.mockResolvedValue({
			error: {
				message: "function upsert_permit_max does not exist",
				code: "42883",
			},
		});
	...
	it("falls back to insert when RPC is missing in schema cache", async () => {
		mockRpc.mockResolvedValue({
			error: {
				message:
					"Could not find the function public.upsert_permit_max in the schema cache",
				code: "PGRST202",
			},
		});
		const context = makeContext();
	...
	it("falls back to insert when RPC permission is denied", async () => {
		mockRpc.mockResolvedValue({
			error: {
				message: "permission denied for function upsert_permit_max",
				code: "42501",
			},
		});
		const context = makeContext();
	...
	it("returns false when RPC fails for other errors", async () => {
		mockRpc.mockResolvedValue({
			error: { message: "data too long", code: "22001" },
		});
		const context = makeContext();
		const paymentModule = new PaymentModule(context);
		const didUpsert = await (
