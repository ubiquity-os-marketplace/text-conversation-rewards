...
describe("GithubCommentModule Simplification Rounding", () => {
	let githubCommentModule: InstanceType<
		typeof import("../src/parser/github-comment-module").GithubCommentModule
	>;

	beforeEach(async () => {
		const { GithubCommentModule } = await import(
	...
	it("should not display floating point noise in Task Simplification total", async () => {
		...
		const bodyContent = await githubCommentModule.getBodyContent(
			{} as unknown as IssueActivity,
			result,
		);

		expect(bodyContent.raw).toContain(
			"<td>Task Simplification</td><td>1</td><td>1.44</td>",
		);
