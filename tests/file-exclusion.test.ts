...
describe("getExcludedFiles tests", () => {
    ...
    `;
		mockGetContent.mockImplementation((params) => {
			if (params?.path === ".gitattributes")
				return mockFileResponse(gitAttribContent);
    ...
    `;
		mockGetContent.mockImplementation((params) => {
			if (params?.path === ".prettierignore")
				return mockFileResponse(prettierIgnoreContent);
			return mockNotFoundError();
		});
		const result = await getExcludedFiles(ctx, owner, repo);
		expect(new Set(result)).toEqual(
			new Set([...defaults, "node_modules", "*.log", "build/"]),
		);
    ...
    `;
		mockGetContent.mockImplementation((params) => {
			if (params?.path === "tsconfig.json")
				return mockFileResponse(tsconfigContent);
			return mockNotFoundError();
		});
		const result = await getExcludedFiles(ctx, owner, repo);
		expect(new Set(result)).toEqual(
			new Set([...defaults, "node_modules", "dist", "**/*.spec.ts", "bin/"]),
		);
