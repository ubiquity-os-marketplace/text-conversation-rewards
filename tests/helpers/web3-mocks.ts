...
export function mockWeb3Module(modulePath?: string) {
	const mocks = {
		Erc20Wrapper: {
			getBalance: jest
				.fn()
				.mockReturnValue(parseUnits("20000", 18) as BigNumber),
		...
		Permit2Wrapper: {
			...
			estimatePermitTransferFromGas: jest
				.fn()
				.mockReturnValue(parseUnits("0.02", 18)),
	...
	jest.mock(modulePath ?? "../../src/helpers/web3", () => {
		...
		class MockPermit2Wrapper {
			generateBatchTransferPermit =
				mocks.Permit2Wrapper.generateBatchTransferPermit;
			sendPermitTransferFrom = mocks.Permit2Wrapper.sendPermitTransferFrom;
			estimatePermitTransferFromGas =
				mocks.Permit2Wrapper.estimatePermitTransferFromGas;
