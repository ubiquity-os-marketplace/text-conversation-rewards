...
import {
	customEncodePermits,
	generatePermitUrlPayload,
} from "../__mocks__/local-permits";
...
jest.mock("@ubiquity-os/permit-generation", () => {
	...
	return {
		...
		encodePermits: (obj: Array<Record<string, unknown>>) =>
			customEncodePermits(obj),
						...
						getWalletByUserId: jest.fn((userId: number) => {
							...
							if (!wallet) {
								return Promise.resolve(
									`[mock] Could not find wallet for user ${userId}`,
								);
							}
